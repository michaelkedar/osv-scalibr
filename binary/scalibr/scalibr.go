...
import (
	...
	"github.com/google/osv-scalibr/binary/fixrunner"
	fixcli "github.com/google/osv-scalibr/binary/fixrunner/cli"
...
func main() {
	...
	switch subcommand {
	case "fix":
		flags, err := fixcli.ParseFlags(os.Args[2:])
		if err != nil {
			log.Errorf("Error parsing CLI args: %v", err)
			os.Exit(1)
		}
		os.Exit(fixrunner.RunFix(flags))
	case "scan":
		flags := parseFlags(os.Args[2:])
		os.Exit(scanrunner.RunScan(flags))
	default:
		// Assume 'scan' if subcommand is not recognized.
